...
pub fn generate_prompt(conn: &Connection, cfg: &Config) -> TdgResult<String> {
    ...
    sections.push("".to_string());
    sections.push("*This is your primary navigation input. Read the terrain, then navigate.*".to_string());
    sections.push("".to_string());
    ...
    if let Some(anbt) = terrain.get("active_nodes_by_type").and_then(|v| v.as_object()) {
        let mut sorted: Vec<_> = anbt.iter().collect();
        sorted.sort_by(|a, b| b.1.as_i64().unwrap_or(0).cmp(&a.1.as_i64().unwrap_or(0)));
        let type_parts: Vec<String> = sorted.iter().take(8)
            .map(|(k, v)| format!("{}: {}", k, v))
            .collect();
    ...
    if let Some(items) = terrain.get("highest_relevance_items").and_then(|v| v.as_array()) {
        if !items.is_empty() {
    ...
    sections.push("".to_string());
    let wm_data = working_memory.get("working_memory").unwrap_or(&working_memory);
    let task = wm_data.get("current_project")
        .and_then(|v| v.as_str())
    ...
    sections.push("".to_string());
    sections.push("1. **Read the terrain context first** — what's happening matters most".to_string());
    sections.push("2. **Consult the diagnostic dashboard** — use pattern flags as awareness, not constraints".to_string());
    sections.push("3. **Choose ONE action** based on terrain relevance, not quadrant compliance".to_string());
    ...
    sections.push("6. **Tag the action** with its natural expression (drive + quadrant) as a post-hoc label".to_string());
    ...
    sections.push("".to_string());
    sections.push("**You are the navigator. The terrain and the dashboard are your maps.**".to_string());
    sections.push("".to_string());
...
mod tests {
    ...
    fn generate_prompt_lean_mode() {
        let conn = setup();
        let mut cfg = Config::with_db_path(tempfile::NamedTempFile::new().unwrap().into_temp_path().to_path_buf());
        cfg.lean = true;
        let prompt = generate_prompt(&conn, &cfg).unwrap();
