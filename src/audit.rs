...
/// Export audit bundle as Markdown
pub fn export_audit_markdown(bundle: &AuditBundle) -> String {
    let mut lines = vec![
        "# TDG Audit Report v2.0".to_string(),
        format!("Generated: {}", bundle.generated_at),
        format!(
            "Nodes: {} | Edges: {} | Events: {}",
            bundle.node_count, bundle.edge_count, bundle.event_count
        ),
        String::new(),
        format!("## Health: {}", bundle.overall_health.status.to_uppercase()),
    ];

    for issue in &bundle.overall_health.issues {
        lines.push(format!("- ⚠️ {}", issue));
    }

    // Integrity
    if let Some(integrity) = bundle.reports.get("integrity") {
        ...
        if let Some(anomalies) = integrity.get("anomalies").and_then(|v| v.as_array()) {
    ...
    // Polarity
    if let Some(polarity) = bundle.reports.get("polarity") {
        lines.push(String::new());
        lines.push("## 2. Polarity".to_string());
        if let Some(entropy) = polarity.get("entropy") {
            lines.push(format!(
                "- Entropy: {} ({})",
                entropy
    ...
    // Stage
    if let Some(stage) = bundle.reports.get("stage_progression") {
        lines.push(String::new());
        lines.push("## 3. Stage Progression".to_string());
        lines.push(format!(
            "- Valid: {}",
            if stage
                .get("hierarchy_valid")
        ...
        lines.push(format!(
            "- Distribution: {:?}",
            stage.get("stage_distribution")
        ));
    }

    // Persistence
    if let Some(persistence) = bundle.reports.get("persistence") {
        lines.push(String::new());
    ...
    // Capabilities
    if let Some(capability) = bundle.reports.get("capability") {
        lines.push(String::new());
    ...
    // Chronic anomalies
    if !bundle.chronic_anomalies.is_empty() {
        lines.push(String::new());
        lines.push("## 6. Chronic Anomalies".to_string());
...
mod tests {
    ...
    #[test]
    fn export_markdown_produces_output() {
        let conn = setup();
        let tmp = tempfile::tempdir().unwrap();
        let engine = AuditEngine::new(&conn, &tmp.path().join("anomalies.json"));
        let bundle = engine.full_audit_bundle().unwrap();
        let md = export_audit_markdown(&bundle);
        assert!(md.contains("TDG Audit Report v2.0"));
